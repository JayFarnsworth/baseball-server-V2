...
app.get('/dailyobject', function (req, res) {
  if (req.query.date) {
    var date = req.query.date;
  } else {
    var date = getDate();
  }
  fetch('http://localhost:4000/rosterplayers')
  .then(fetch('http://localhost:4000/games'))
  .then(fetch('http://localhost:4000/getalllineups'))
  .then(fetch('http://localhost:4000/formatgameobject'))
  .then(fetch('http://localhost:4000/eliminatebadgames'))
  .then(resp=>resp.json())
    ...
    let gamesNew = gameObj.map(game=>{
      var homeLineup = game.homeTeam.lineup.map(player=>{
        if (player.info && player.info.ID) {
          ...
          console.log('WARN: no player.info', player)
        }
        return player
      })
      homeLineup = homeLineup.filter(player => player && player.ID)
